...
class AuthUtility extends AbstractUtility {
    ...
    static isAuthenticated(req) {
        App.instance.logger.log(App.instance.auth.scopes)
        App.instance.logger.log(req.originalUrl.replace("/api/", ""))

        const methodType = (req.method === 'GET') ? 'read' : 'write';
        const url = req.originalUrl.replace("/api/", "")


        App.instance.logger.log(methodType + ':' + url);

        App.instance.logger.log(App.instance.auth.scopes.includes(methodType + ':' + url))
